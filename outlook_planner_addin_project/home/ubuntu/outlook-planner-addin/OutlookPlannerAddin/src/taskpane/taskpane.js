...
async function getAndAttachEmailAsEml(token, taskId, emailSubject) {
    ...
    try {
        // 1. Get EML data using Office.js
        const item = Office.context.mailbox.item;
        const emlDataResult = await new Promise((resolve, reject) => {
        ...
        const base64Eml = emlDataResult; // This is already base64
        ...
        const safeFileName = (emailSubject || "email").replace(/[^a-z0-9_.-]/gi, '_') + ".eml";

        // 2. Upload .eml to OneDrive (e.g., to a specific folder, or user's root for simplicity in MVP)
        // For MVP, uploading to /Attachments/OutlookPlannerAddin/{taskId}/{safeFileName}.eml
        const uploadPath = `Attachments/OutlookPlannerAddin/${taskId}/${safeFileName}`;
        console.log(`Uploading .eml to OneDrive: /me/drive/root:/${uploadPath}:/content`);

        const uploadResponse = await fetch(`${graphApiEndpoint}/me/drive/root:/${uploadPath}:/content`, {
            method: "PUT",
            headers: {
                "Authorization": "Bearer " + token,
        ...
        const referencePayload = {
            ...
            "alias": safeFileName,
            "previewPriority": " !", // Default preview priority
            "type": "Other", // Could be "Outlook" or a custom type
            "resourceUrl": uploadedFile.webUrl // Link to the file in OneDrive
        };
        ...
        if (!detailsResponse.ok) throw new Error("Failed to get task details for ETag before adding reference.");
        const etag = detailsResponse.headers.get("ETag");
        ...
        const addReferenceResponse = await fetch(`${graphApiEndpoint}/planner/tasks/${taskId}/details`, {
            ...
            headers: {
                ...
                "Accept": "application/json",
                "If-Match": etag || "*",
                "Prefer": "return=representation" // To get the updated details back
            },
            body: JSON.stringify({ 
                references: { 
                    [uploadedFile.webUrl]: referencePayload // Use URL as key for the reference object
                }
            })
        });
        ...
        updateStatus(`Task created, but error attaching email: ${error.message}`, true);
        throw error; // Re-throw to be caught by createTask's final catch
    }
}


// Function to update the status message area in the task pane
